...
fit.rf <- train(Survived~ Pclass + Sex + Age + SibSp + Parch + 
                                            Fare + Embarked + Title + 
                                            FsizeD + Child, data=train1, method="svmLinear",ntree = 1700,trControl=control)
```
```{r}
fit.rf$results
```
...
prediction <- predict(fit.rf, test1)
 ...
 (prediction)
solution <- data.frame(PassengerID = pasidtest, Survived = prediction)

write.csv(solution, file = 'rf_solution_svm.csv', row.names = F)
```
